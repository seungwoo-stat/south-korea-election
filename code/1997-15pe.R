...
## Data generation -------------------------------------------------------------
data_real <- tibble()
filenames <- list.files("../raw/1997-15pe")
for(i in seq_along(filenames)){
  print(i)
  print(filenames[i])
  x <- read_excel(paste0("../raw/1997-15pe/",filenames[i]), skip = 6, col_names = FALSE)[,-c(2:3,8,12)]
  colnames(x) <- c("secondary_cluster",
                   "Lee Hoi-chang	(Grand National)",
                   "Kim Dae-jung (National Congress for New Politics)",
                   "Lee In-je	(New National)",
                   "Kwon Young-ghil	(People's Victory for 21st Century)",
                   "Huh Kyung-young	(Republican)",
                   "Kim Han-sik	(Honest Politics Unite)",
                   "Shin Jeong Yil (Unified Korea)",
                   "invalid",
                   "blank")
  m <- nrow(x)
  for(j in 2:8){
...
# save as csv
write.csv(data_real,file="../csv/15pe.csv", row.names = FALSE, fileEncoding = "UTF-8")

## -----------------------------------------------------------------------------
data_real <- read.csv("../csv/15pe.csv")
